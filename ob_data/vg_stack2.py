...
listpath='/data/work/virgo3d/plots_badsky/'
...
x0 = 11.
y0 = 43.
anns = np.array([0.,3.,6.,10.,15.,25.,40.])
###
...
for ind_ann in range(flux_stack.shape[1]-1):
    ...
    tmpflux2 = ndimage.gaussian_filter1d(tmpflux, 4)
...
#plt.show()
plt.savefig(stack_dir+name+'_ann_stack2.png')
...
hdu = fits.BinTableHDU.from_columns(
      [fits.Column(name='wave',format='E',array=wave/(1+rz)),
       ...
       fits.Column(name='flux4',format='E',array=flux_stack[:,3]),
       fits.Column(name='flux5',format='E',array=flux_stack[:,4]),
       fits.Column(name='flux6',format='E',array=flux_stack[:,5]),
       fits.Column(name='flux_all',format='E',array=flux_stack[:,6])])
hdu.writeto(stack_dir+name+'_ann_stack2.fits',overwrite=True)
